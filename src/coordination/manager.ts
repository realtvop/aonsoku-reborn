...
import { CoordinationHttpClient, buildSubsonicProof, type CoordinationCredentials, type StoredDeviceTokens } from "./httpClient";
import { CoordinationWsClient, type ConnectionState, type ConnectionCallbacks } from "./wsClient";
import { HistoryOutbox } from "./outbox";
import { clearTokens, loadConfig, loadTokens, saveConfig, saveTokens, type CoordinationConfig } from "./tokenStore";
...
export interface CoordinationManagerCallbacks {
	...
	onDeviceSnapshot: (deviceId: DeviceId, snapshot: PlaybackSnapshot, isOnline: boolean) => void;
	onRemoteCommand: (command: RemoteCommand, sourceDeviceId: DeviceId) => void;
	onHandoffCandidate: (snapshot: PlaybackSnapshot, transactionId: string) => void;
	onPrepareRelinquish: (transactionId: string, expectedSnapshotRevision: SnapshotRevision) => void;
	onHandoffCommitted: (snapshot: PlaybackSnapshot, newGeneration: SessionGeneration) => void;
...
export class CoordinationManager {
	...
	private capabilities = CoordinationCapability.HISTORY | CoordinationCapability.OBSERVE | CoordinationCapability.CONTROL | CoordinationCapability.HANDOFF;
	...
	async connect(creds: CoordinationCredentials, deviceName: string, platform: string, clientVersion: string): Promise<void> {
		if (!this.config) throw new Error("coordination: server URL not configured");
		this.httpClient = new CoordinationHttpClient(this.config.serverUrl);
	...
	private async openWebSocket(): Promise<void> {
		...
		const wsUrl = this.config!.serverUrl.replace(/^http/, "ws") + "/v1/realtime";
		const cb: ConnectionCallbacks = {
			...
			onSnapshotProjection: (env) => {
				if (env.type === "snapshot_projection") {
					this.callbacks.onDeviceSnapshot(env.deviceId, env.snapshot, env.isOnline);
				}
			},
			onCommand: (env) => {
				if (env.type === "command") {
			...
			onPrepareRelinquish: (env) => {
				if (env.type === "prepare_relinquish") {
					this.callbacks.onPrepareRelinquish(env.transactionId, env.expectedSnapshotRevision);
				}
			},
	...
	async enqueueHistoryOperation(operation: HistoryOperationInput): Promise<void> {
	...
	private async flushOutbox(): Promise<void> {
		...
		try {
			const resp = await this.httpClient.pushHistory(due.map((e) => e.operation));
	...
	async legacyImport(entries: { songId: string; songTitle?: string; songArtist?: string; songAlbum?: string; songDuration?: number }[]): Promise<{ mergedSongIds: string[]; isFirstDevice: boolean }> {
		if (!this.httpClient) throw new Error("coordination: not connected");
	...
	publishSnapshot(
		...
		this.wsClient?.publishSnapshot(sessionId, generation, snapshotRevision, snapshot);
	}

	sendCommand(targetDeviceId: DeviceId, expectedGeneration: SessionGeneration, command: RemoteCommand) {
		this.wsClient?.sendCommand(targetDeviceId, expectedGeneration, command);
	}

	requestHandoffCandidate(sourceDeviceId: DeviceId, expectedGeneration: SessionGeneration, expectedSnapshotRevision: SnapshotRevision) {
		this.wsClient?.requestHandoffCandidate(sourceDeviceId, expectedGeneration, expectedSnapshotRevision);
	}

	sendTargetReady(transactionId: string, generation: SessionGeneration, snapshotRevision: SnapshotRevision) {

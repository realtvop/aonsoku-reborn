...
export interface ConnectionCallbacks {
	...
	onWelcome: (deviceId: DeviceId, connectionId: ConnectionId, negotiated: CoordinationCapabilities) => void;
...
export class CoordinationWsClient {
	...
	private negotiatedCaps: CoordinationCapabilities = CoordinationCapability.NONE;
	...
	private startHeartbeat() {
		...
		this.heartbeatTimer = setInterval(() => {
			this.send({ version: COORDINATION_PROTOCOL_VERSION, messageId: crypto.randomUUID(), type: "heartbeat" });
	...
	private handleEnvelope(env: Envelope) {
		switch (env.type) {
			case "welcome":
				...
				this.callbacks.onWelcome(env.deviceId, env.connectionId, env.negotiated);
				break;
			case "heartbeat_ack":
			...
			case "capability_disabled":
				this.callbacks.onError("protocol_incompatible", `feature disabled: ${env.feature}`);
				break;
			default:
				break;
		}

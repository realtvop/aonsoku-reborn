...
export class LanControlServer {
    ...
    async start(): Promise<{ success: boolean; port?: number; error?: string }> {
        try {
            ...
            this.wss.on('connection', (ws: WebSocket, req: IncomingMessage) => {
                console.log('[LAN Control] New client connected from', req.socket.remoteAddress)
                    ...
                    try {
                        ...
                        if (msg.type === LanControlMessageType.AUTH_REQUEST) {
                            ...
                            const response: LanControlMessage = {
                                ...
                                data: {
                                    success: authResult,
                                    message: authResult ? i18n.en.auth.success : i18n.en.auth.failed,
    ...
    private async authenticate(authData: AuthRequestData): Promise<boolean> {
        if (authData.authType === 'lan') {
            ...
            return authData.password.toUpperCase() === this.config.password.toUpperCase()
        } else if (authData.authType === 'navidrome' && this.config.allowNavidromeAuth) {

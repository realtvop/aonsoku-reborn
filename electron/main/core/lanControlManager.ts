...
export class LanControlManager {
    ...
    private setupIpcHandlers(): void {
        // Start server
        ipcMain.handle('lan-control:start', async (_event, config: LanControlConfig) => {
        ...
        // Update config
        ipcMain.handle('lan-control:update-config', async (_event, config: LanControlConfig) => {
            return await this.updateConfig(config)
        })

        // Broadcast state
        ipcMain.on('lan-control:broadcast-state', (_event, state: PlayerStateData) => {
            this.broadcastState(state)
        })

        // Broadcast song
        ipcMain.on('lan-control:broadcast-song', (_event, song: CurrentSongData) => {
